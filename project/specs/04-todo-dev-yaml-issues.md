# [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/) and issues in Markdown
...
- Issues folder: [`yoda/project/issues/`](../yoda/project/issues/)
...
## Minimal schema for [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/)
...
- pending -> to-do or doing (via [`pending_resolve.py`](../yoda/scripts/pending_resolve.py))

## Benefits

- Single source of truth for work state.
- Bulk operations are straightforward (list, filter, reorder).
- Token savings by sending only [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/) for planning.
- Markdown stays free for narrative detail.

## Minimal identifier

The issue Markdown file must have a minimal identifier to link to [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/), for example:

- Use the ID in the Markdown title.

## Notes

- The AI must not edit [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/) directly; it must call scripts.
- The AI can freely edit the issue text.

## Issue metadata (Markdown)

Issue files use YAML front matter that mirrors the issue metadata in [`yoda/todos/TODO.<dev>.yaml`](../yoda/todos/).
