...
- Issue file name: `<id>-<slug>.md`
  ...
  - slug = filename segment used for readability; metadata source of truth is the issue id plus front matter.
...
- Optional empty fields (`depends_on`, `pending_reason`, `extern_issue_file`) should be omitted from YAML/front matter.
- The filename slug can be changed through `todo_update.py` rename flows (`--title` recalculation or explicit `--slug`).
