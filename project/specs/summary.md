...
- Artifacts direction: one TODO per developer ([`yoda/todos/TODO.<dev>.yaml`](/yoda/todos/)) and one Markdown file per issue for richer free-form descriptions, with scripts generating the issue skeleton.
- Issues location and naming: issues live in [`yoda/project/issues/`](/yoda/project/issues/) and follow `<id>-<slug>.md`.
...
- Brand voice and terminology: clear, technical, pragmatic; English for canonical specs; standardized terms (YODA Framework, YODA Flow, issue, [`yoda/todos/TODO.<dev>.yaml`](/yoda/todos/), agent, script).
...
- Scripts: live in [`yoda/scripts/`](/yoda/scripts/), written in Python, file name is the command; [`init.py`](/yoda/scripts/init.py) is mandatory.
...
- Logs: one YAML log per issue at [`yoda/logs/<id>-<slug>.yaml`](/yoda/logs/).
- Bootstrap exception: while scripts do not exist, use [`yoda/todos/TODO.<dev>.md`](/yoda/todos/) and Markdown logs at [`yoda/logs/<id>-<slug>.md`](/yoda/logs/); bootstrap is temporary.
