...
* **MUST** review doc/spec changes via **pull requests** (or equivalent review mechanism), not “direct edits on main.” ([GitHub Docs][2])
...
* **MUST** run automated validation in CI for YODA artifacts (at minimum: schema checks for YAML, reference/link integrity, and required-section checks). ([Write the Docs][1])
