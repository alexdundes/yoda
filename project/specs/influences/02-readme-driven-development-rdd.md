...
* **MUST** provide a **primary human entrypoint** README for the framework (the “front door”) that answers: *what it is, why it exists, quickstart, and where the specs live*. The **agent entrypoint** is `yoda/yoda.md`. ([Tom Preston-Werner][1])
* **MUST** include a **real Quickstart** in the README (or link to a Quickstart doc) that results in a working minimal flow. ([Appsmith][3])
