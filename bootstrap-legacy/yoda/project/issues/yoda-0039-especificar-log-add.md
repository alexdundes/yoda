...
depends_on: []
description: Especificar o comportamento do `log_add.py` e produzir a spec em `project/specs/19-log-add-script.md`.
entrypoints: []
id: yoda-0039
lightweight: false
origin:
...
# yoda-0039 - Especificar script de log (log_add.py)
<!-- AGENT: Replace yoda-0039 with the canonical issue id (dev-id, e.g., yoda-0001) from `yoda/todos/TODO.<dev>.yaml` and Especificar script de log (log_add.py) with the issue title. Fill front matter fields from TODO; scripts must keep them in sync. Keep any <...> placeholders wrapped in inline code when used in prose. -->
